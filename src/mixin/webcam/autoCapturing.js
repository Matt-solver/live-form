...
export function autoCapture(self) {
  ...
  function gotResult(err, results) {
    ...
    for (let e of edge) {
      if (document.querySelector(e)) document.querySelector(e).classList.remove('shake');
    }
    if (!results) return;
  ...
  function faceTracking(self, queryImage) {
    ...
    let streamDetecting = setInterval(async function() {
      ...
      faceObj = await faceapi
        .detectSingleFace(self.video, new faceapi.TinyFaceDetectorOptions({ inputSize: 320 }))
        .withFaceLandmarks()
        .withFaceDescriptor();
      ...
      if (faceObj) {
        ...
        if (bestMatch) {
          if (bestMatch.distance) {
            ...
            if (Math.round(bestMatch.distance * 100) / 100 <= 0.9) {
  ...
  function stopVideo(streamDetecting) {
    ...
    if (streamDetecting) clearInterval(streamDetecting); // stop roof
    self.video.pause();
  }
