...
export const StepToTakeIdPhoto = {
  data() {
    return {
      stepType: 'id',
      step: 2,
  ...
  async mounted() {
    console.log('%c' + '🔥🔥🔥🔥🔥 STEP 2 ID still cut 🔥🔥🔥🔥🔥', 'color:blue;font-weight:bold;');
    ...
    if (window.orientation === 0) {
      ...
      this.height_id = screen.offsetWidth * viewportRatio * 0.85 * this.ID_type_Rate;
    }
    ...
    if (this.$store.state.ver !== 'automatic') this.$store.state.isIdentityDocument = true;

    this.$store.state.is_iOS || (await this.playWebcam(this, 'id'));
    this.$store.state.is_iOS && this.playLocalCam(this);
    this.$store.state.is_iOS || (this.$store.state.ver === 'automatic' && this.autoCapture(this));
  },
  watch: {
    resizeFlag: function() {
  ...
  methods: {
    ...
    onButton() {
      ...
      this.btn_flag = true;
      this.shot_btn = true;
      if (this.$refs.shotBtn) this.$refs.shotBtn.classList.remove('opacity');
    },
    offButton() {
      console.log('✔️offButton');
      this.btn_flag = false;
      this.shot_btn = true;
      if (this.$refs.shotBtn) this.$refs.shotBtn.classList.add('opacity');
    },
    onVideo() {
    ...
    next_step: async function() {
      ...
      /****** Prevent CSRF *****
            ...
            /******/
      console.info('In case of Passport, do not take photos of the back side && no Backside Image');
      if (
        // 신분증 뒷면 촬영시 임시 분기조건
        ...
        let params = new DTO.DatasetParams({
          ...
          issuingCountry: this.Issue_country_code ? this.Issue_country_code : '',
    ...
    capture: function() {
      ...
      if (!self.$store.state.is_iOS) self.image = self.c1.toDataURL('image/png');
